...
export class Router {
    ...
    async route(request) {
        ...
        const dimensions = [
            ...
            this.scoreKeywordDimension(userText, 'codePresence', 'code', this.getCategoryKeywords('MEDIUM'), { low: 1, high: 2 }, { none: 0, low: 0.5, high: 1.0 }),
            this.scoreKeywordDimension(userText, 'reasoningMarkers', 'reasoning', this.getCategoryKeywords('REASONING'), { low: 1, high: 2 }, { none: 0, low: 0.7, high: 1.0 }),
            this.scoreKeywordDimension(userText, 'technicalTerms', 'technical', this.getCategoryKeywords('COMPLEX'), { low: 2, high: 4 }, { none: 0, low: 0.5, high: 1.0 }),
            this.scoreKeywordDimension(userText, 'creativeMarkers', 'creative', this.getCategoryKeywords('CREATIVE'), { low: 1, high: 2 }, { none: 0, low: 0.5, high: 0.7 }),
            this.scoreKeywordDimension(userText, 'simpleIndicators', 'simple', this.getCategoryKeywords('SIMPLE'), { low: 1, high: 2 }, { none: 0, low: -1.0, high: -1.0 }),
    ...
    checkOverrides(request) {
        ...
        if (estimatedTokens > 25000) {
            ...
            if (cat)
                return { models: cat.models, name: cat.name, reason: `Large context (${estimatedTokens} tokens)` };
        }
    ...
    scoreTokenCount(estimatedTokens) {
        if (estimatedTokens < 50)
            return { name: 'tokenCount', score: -1.0, signal: `short (${estimatedTokens}t)` };
        if (estimatedTokens > 500)
            return { name: 'tokenCount', score: 1.0, signal: `long (${estimatedTokens}t)` };
        return { name: 'tokenCount', score: 0, signal: null };
    }
    /** Dimension 2-6: Keyword-Matching gegen Kategorie-Keywords */
    scoreKeywordDimension(text, name, signalLabel, keywords, thresholds, scores) {
    ...
    scoreImperativeVerbs(text) {
        ...
        if (matches.length >= 2)
            return { name: 'imperativeVerbs', score: 0.5, signal: `imperative (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'imperativeVerbs', score: 0.3, signal: `imperative (${matches[0]})` };
    ...
    scoreConstraints(text) {
        ...
        if (matches.length >= 3)
            return { name: 'constraintCount', score: 0.7, signal: `constraints (${matches.slice(0, 3).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'constraintCount', score: 0.3, signal: `constraints (${matches[0]})` };
    ...
    scoreOutputFormat(text) {
        ...
        if (matches.length >= 2)
            return { name: 'outputFormat', score: 0.7, signal: `format (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'outputFormat', score: 0.4, signal: `format (${matches[0]})` };
        return { name: 'outputFormat', score: 0, signal: null };
    }
    /** Dimension 12: Reference Complexity */
    scoreReferences(text) {
        ...
        if (matches.length >= 2)
            return { name: 'referenceComplexity', score: 0.5, signal: `references (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'referenceComplexity', score: 0.3, signal: `references (${matches[0]})` };
    ...
    scoreNegation(text) {
        ...
        if (matches.length >= 3)
            return { name: 'negationComplexity', score: 0.5, signal: `negation (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 2)
            return { name: 'negationComplexity', score: 0.3, signal: `negation (${matches.slice(0, 2).join(', ')})` };
        return { name: 'negationComplexity', score: 0, signal: null };
    }
    /** Dimension 14: Domain-Specific Keywords */
    scoreDomainSpecific(text) {
        const domains = [
        ...
        if (matches.length >= 2)
            return { name: 'domainSpecificity', score: 0.8, signal: `domain (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'domainSpecificity', score: 0.5, signal: `domain (${matches[0]})` };
        return { name: 'domainSpecificity', score: 0, signal: null };
    }
    /** Dimension 15: Agentic Task */
    scoreAgenticTask(text) {
        const agenticKeywords = this.getCategoryKeywords('AGENTIC');
        const matches = agenticKeywords.filter(kw => text.includes(kw));
        if (matches.length >= 4)
            return { name: 'agenticTask', score: 1.0, signal: `agentic (${matches.slice(0, 3).join(', ')})` };
        if (matches.length >= 2)
            return { name: 'agenticTask', score: 0.6, signal: `agentic (${matches.slice(0, 2).join(', ')})` };
        if (matches.length >= 1)
            return { name: 'agenticTask', score: 0.2, signal: `agentic-light (${matches[0]})` };
